...
while read line; do
	...
	then
		#Extract synonymous sites that have at least 20x coverage
 		perl SCRIPTS/Population_STRUCTURE/snpeff_and_structure/compare_snpeff_filtered_to_SNPs20x.pl $WORKING_DIR/$SYNONYMOUS_DIR/$line\_snpEff_syn_snps.txt $line/SNPs/20X/$line\_SNP_freq_20x.txt > $WORKING_DIR/$SYNONYMOUS_DIR/Filtered/$line\_snpeff_syn_snps_filt.txt
 		echo $line " done!"
 	else
